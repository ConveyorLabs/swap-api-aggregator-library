// for each chain ID, we have a list of supported protocols, where the key is the protocol ID and the value is the protocol name mapped to the protocol ID. We can use this mapping to filter out the protocols we want to include or exclude in the query. We can also use this mapping to display the protocol names in the UI.

// when adopting to your own project, you can replace the supportedProtocols object with your own mapping of protocol IDs to protocol names and have the value be the protocol name mapped to the protocol ID. You can also add more chain IDs and their corresponding supported protocols to the supportedProtocols object.

// keep the values as closely as possible to the original values to avoid having duplicates in https://swap-api.xyz/sources/[chainId]
...
//Do not alter the code below, it automatically generates the query based on the supportedProtocols object and whats passed into the url query

// Eg:http://localhost:3000/quote?tokenIn=0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE&tokenOut=0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48&amountIn=1000000000000000000&slippage=1&chainId=1&recipient=0x92977D2552f455Bb9A3457AEbfCb78f1256Dd2e5&includeProtocols=uniswap-v3
